...
import octo.content.OctoUI;
import octo.util.Regions;
import octo.util.ThrowableUtils;
...
//                modInfo.meta.description = descriptionBuilder.toString();
            if(Core.scene != null && !headless){
                ...
                if(dialog instanceof BaseDialog &&
                        Objects.equals(dialog.title.getText().toString(), mod.meta.displayName()))
                {
    ...
    public ModDialog() {
        ...
        Seq<UnlockableContent> all = Seq.with(content.getContentMap()).<Content>flatten()
                .select(c -> c.minfo.mod == mod && c instanceof UnlockableContent).as();
        ...
        this.cont.table(md -> {
            ...
            md.pane(desc -> {
                ...
                desc.add(mod.meta.author).growX().wrap().padTop(0).row();
                desc.row();

                desc.add("@editor.description").padRight(10).color(Color.gray).top().row();
                desc.add(mod.meta.description).growX().wrap().padTop(0).row();
        ...
        if(!mobile) {
            this.buttons.button("@mods.openfolder", Icon.folder, () -> {
                Core.app.openFolder(mod.root.absolutePath());
            });
        }
    }
}